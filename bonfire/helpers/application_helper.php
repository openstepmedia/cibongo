...
if (!function_exists('gravatar_link')) {
    ...
    function gravatar_link($email = null, $size = 48, $alt = '', $title = '', $class = null, $id = null)
    {
        ...
        if (empty($email)) {
            ...
            $avatarURL = sprintf(
                    $gravatarUrl, md5(strtolower(trim($email))), $size, strtolower($rating), rawurlencode($defaultImage)
            );
        }
...
if (!function_exists('log_activity')) {
    ...
    function log_activity($userId = null, $activity = '', $module = 'any')
    {
        ...
        if ($ci->config->item('enable_activity_logging') === true) {
            $ci->load->model('activities/activity_odm_model');
            return $ci->activity_odm_model->log_activity($userId, $activity, $module);
        }
        return false;
    }
