 ...
 */
class User_odm_model extends BF_ODM_Model
{
    /** @var string Name of the users table. */
    protected $repository = 'bonfire\modules\users\models\User';
...
*/
    ...
    {
        //total records in collection - for pagination
        if ($get_deleted) {
            $qb = $this->qb()->field('deleted')->notEqual(0);

            // Get only the deleted users
            //$this->db->where("{$this->table_name}.deleted !=", 0);
        } else {
            $qb = $this->qb()->field('deleted')->equals(0);
            //$this->db->where("{$this->table_name}.deleted", 0);
        }
     ...
     */
    public function insert($data = array(), $return_type = 'id')
    {
        // If 'display_name' is not provided, set it to 'username' or 'email'.
        if (empty($data['display_name'])) {
            if ($this->settings_odm_lib->item('auth.use_usernames') == 1
                && ! empty($data['username'])
            ) {
    ...
    public function set_to_default_role($current_role)
    {
        ...
        $defaultRoleId = $this->role_odm_model->default_role_id();
    ...
    public function clear_login_attempts($login, $expires = 86400, $ip_address = null)
    {
