 ...
 */
class Role_odm_model extends BF_ODM_Model
{
    /** @var string Name of the table. */
    protected $repository = 'bonfire\modules\roles\models\Role';
    ...
    public function update($id = null, $data = null)
    {
        ...
        //convert list of IDs to DBRef
        if(!empty($data['role_permissions'])) {
            $data['role_permissions'] = $this->get_permission_dbrefs($data['role_permissions']);
        }
        
        return parent::update($id, $data);
    }

    public function get_permission_dbrefs($role_permissions) 
    {
        $role_permissions_dbrefs = array();
        $this->load->model('permissions/permission_odm_model');
        foreach($role_permissions as $id) {
            $dbref = $this->permission_odm_model->find($id);
            if(!empty($dbref->id)) {
                $role_permissions_dbrefs[] = $dbref;
            }
        }
        return $role_permissions_dbrefs;
    }
    ...
    public function get_all_permissions()
    {
        ...
        $permissions = $this->permission_odm_model->qb()
    ...
    public function get_role_permissions(&$role)
    {
        ...
        $permission_array = array();
        $role_permissions = $this->role_permission_model->find_for_role($role->id);
