 ...
 */
class BF_ODM_Model extends CI_Model
{
    ...
    public function qb()
    {
        ...
        return $this->doctrineodm->dm
                ->createQueryBuilder($this->repository);
    }
    ...
    public function find($id = '')
    {
        ...
        $document = $this->doctrineodm->dm->find($this->repository, $id);
        ...
        $return = $this->trigger('after_find', $document);
    ...
    public function find_by($field = '', $value = '', $type = 'and')
    {
        ...
        $return = $this->doctrineodm->dm->getRepository($this->repository)->findOneBy($where);
     ...
     */
    public function insert($data = null, $return_type = 'id')
    {
    ...
    public function update($where = null, $data = null)
    {
        ...
        $this->trigger('after_update', array($data, $document));
        return true;
 ...
 */
    ...
    public function get_field_info()
    {
        if (empty($this->field_info)) {
            $this->field_info = $this->doctrineodm->dm
                    ->getMetadataFactory()
                    ->getMetadataFor($this->repository)
                    ->fieldMappings;
    ...
    public function prep_data($post_data)
    {
        ...
        foreach ($this->get_field_info() as $field) {
            if (isset($field['id'])
                && $field['id']
                || in_array($field['fieldName'], $skippedFields)
                || ! isset($post_data[$field['fieldName']])
            ) {
                continue;
            }

            $data[$field['fieldName']] = $post_data[$field['fieldName']];
        }
    ...
    {
        try {
            $this->doctrineodm->dm->persist($obj);
            $this->doctrineodm->dm->flush();
            return true;
        }
        catch(Exception $e) {
            $this->error = "Could not persist record:" . $e->getMessage();
            return false;
        }
    }    
}
